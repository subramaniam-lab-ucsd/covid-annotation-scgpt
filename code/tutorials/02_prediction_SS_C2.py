...
project_dir = Path(f"/tscc/nfs/home/s5srinivasan/scgpt-project/covid-annotation-scgpt")
data_dir = Path(f"/tscc/nfs/home/s5srinivasan/scratch")
...
celltype_id_labels = adata.obs["celltype_id"].astype("category").cat.codes.values
...
# # Step 2b - Split the studies by batch

# In[40]:


'''
Dict of study names and corresponding batch ids
'''
study_batch_ids = {
    "Bali" : 0, # Bali
    "Lee" : 1, # Lee
...
trainingStudies = ["Wilk", "Bali", "Lee", "SS_C1"]
testStudies = ["SS_C2"]
...
adata_train = adata[adata.obs["batch_id"].isin([study_batch_ids[study] for study in trainingStudies])].copy()
adata_test = adata[adata.obs["batch_id"].isin([study_batch_ids[study] for study in testStudies])].copy()
...
adata_test_raw.write_h5ad(data_dir/"predictionsSS_C2.h5ad")


# In[ ]:


torch.save(best_model.state_dict(), save_dir / "SS_C2_Model.pt")
